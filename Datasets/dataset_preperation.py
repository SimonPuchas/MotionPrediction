...
def standardize_data(X, y):
    ...
    mean_X = X_all.mean(dim=0, keepdim=True)
    std_X = X_all.std(dim=0, keepdim=True)
    
    std_X[std_X == 0] = 1  # avoiding NaNs
    X_standardized = [(x - mean_X) / std_X for x in X]
    y_standardized = [(yy - mean_X) / std_X for yy in y]
    
    return X_standardized, y_standardized

def split_sequences(sequences, test_size=0.2, val_size=0.1):
...
if __name__ == "__main__":
    ...
    # Split the data into train, val, and test sets at the sequence level
    X_train_seq, X_val_seq, X_test_seq = split_sequences(list(zip(X_standardized, y_standardized)))
    ...
    # Save the split and standardized dataset
    torch.save({
        'X_train': X_train, 'X_val': X_val, 'X_test': X_test,
        'y_train': y_train, 'y_val': y_val, 'y_test': y_test
    }, '/home/simon/MotionPrediction/Datasets/lstm_dataset4.pt')
